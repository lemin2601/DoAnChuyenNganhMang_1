...
public class LA extends DistAlg {
    ...
    protected String constructRow2(int g) {
        ...
        if (needToSendReply[g])
            s = s + " Phan hoi den " + Screen.node[g] + ".   ";
        else if (needToSendRelease[g])
            s = s + " Giai phong den " + Screen.node[g] + ".   ";
        return s;
    }
   ...
   // Process status
    ...
    private static final String
        NOREPLY   = "Khong can thiet gui tra loi den ",
        NOREQUEST = "Khong can thiet gui yeu cau den ",
        NORELEASE = "Khong can gui lenh giai phong den ",
        ANYNODE   = "cac nut khac";
}
